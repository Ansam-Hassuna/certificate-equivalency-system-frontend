...
export default function ApplicationSubmissionFlow() {
  ...
  const [submitted, setSubmitted] = useState(false);
    const [previousQualificationId, setPreviousQualificationId] =
      useState("");
                ...
                <div className="application-prerequisite">
                  {!prerequisiteCheck.allowed ? (
                    <div className="application-flow__notice">
                      <strong>
                        {language === "ar"
                          ? "لا يمكن متابعة الطلب"
                          : "Application cannot continue"}
                      </strong>

                      <span>
                        {language === "ar"
                          ? "لا يوجد مؤهل سابق مستوفٍ للمتطلبات المطلوبة لهذا المؤهل."
                          : "No eligible previous qualification was found for this qualification."}
                      </span>
                    </div>
                  ) : eligiblePreviousQualifications.length > 1 ? (
                    <Select
                      label={
                        language === "ar"
                          ? "اختر المؤهل السابق المرتبط بهذا الطلب"
                          : "Select the previous qualification for this application"
                      }
                      value={previousQualificationId}
                      onChange={(event) =>
                        setPreviousQualificationId(
                          event.target.value
                        )
                      }
                      options={eligiblePreviousQualifications.map(
                        (item) => ({
                          value: item.id,
                          label:
                            language === "ar"
                              ? `${item.specialization || item.qualification || "مؤهل سابق"}${
                                  item.university
                                    ? ` — ${item.university}`
                                    : ""
                                }`
                              : `${item.specialization || item.qualificationEn || item.qualification || "Previous qualification"}${
                                  item.universityEn || item.university
                                    ? ` — ${
                                        item.universityEn ||
                                        item.university
                                      }`
                    ...
                    <div className="application-flow__notice">
                      <strong>
                        {language === "ar"
                          ? "تم العثور على المؤهل السابق المطلوب"
                          : "Required previous qualification found"}
                      </strong>

                      <span>
                        {language === "ar"
                          ? selectedPreviousQualification?.specialization ||
                            selectedPreviousQualification?.qualification ||
                            "—"
                          : selectedPreviousQualification?.specialization ||
                            selectedPreviousQualification?.qualificationEn ||
                            selectedPreviousQualification?.qualification ||
                            "—"}
                      </span>
                    </div>
