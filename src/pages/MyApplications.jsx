...
function Content(){const {t, language}=useLanguage();const {user}=useAuth();const navigate=useNavigate();const rows=getLocalizedRequestRows(language).filter((row)=>row.ownerUserId===user?.id);return <ScreenShell title={t("myApplications.title")} description={t("myApplications.description")} icon="document" actions={<Button onClick={()=>navigate("/applications/new")} icon={<Icon name="plus" size={18}/>}>{t("applications.new")}</Button>}><div className="workflow-list">{rows.map(r=><Card key={r.id} className="workflow-list-item"><div><strong>{r.id}</strong><span>{r.qualification} Â· {r.university}</span></div><div><Badge tone="neutral">{r.status}</Badge><Button variant="ghost" size="sm" onClick={()=>navigate(`/applications/${r.id}`)}>{t("common.details")}</Button></div></Card>)}</div></ScreenShell>}
